...
*/
//Trovato da stack, ma ottengo sempre oggi per date1 e date 2
const howManyDays = function (da1, da2) {
  let date1 = da1;
  date1 = new Date();
  let date2 = da2;
  date2 = new Date();

  let difDiTempo = date2.getTime() - date1.getTime();

  let difDiGiorni = difDiTempo / (1000 * 3600 * 24);
...
howManyDays("19.07.2023", "10.07.2023");
/* ESERCIZIO 10
  Scrivi una funzione chiamata "isTodayMyBirthday" che deve ritornare true se oggi è il tuo compleanno, falso negli altri casi.
*/
const isTodayMyBirthday = function (n) {
  let ddo = new Date();
  console.log(ddo);
  let compleanno = n;
...
isTodayMyBirthday("Fri Sep 15 2023");
// Arrays & Oggetti
...
const rimasto = deleteProp(myobj, "snorlax");
...
const newestMovie = function () {
  ...
  for (let i = 0; i < movies.length; i++) {
    nuovoArr.push(movies[i].Year);
  }
  let oldest = nuovoArr.sort();
  let newMow = oldest.reverse();
  return newMow;
};
...
const onlyInLastMillennium = function () {
  const nuovoArr = [];
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Year <= 2000) nuovoArr.push(movies[i].Year);
  }
  return nuovoArr;
};
...
*/
const searchByTitle = function (lugia) {
  let cerco = lugia
  const nellaLista = []
  for (let i = 0; i < movies.length; i++) {
    const titolo = movies[i].Title.
    if (movieTitle.includes(cerco())) {
      nellaLista.push(movies[i]); 
    }
  }
  return nellaLista
}
